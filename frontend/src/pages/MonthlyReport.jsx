          ...
          <Card className="earms-card" data-testid="license-fee-section">
            ...
            <CardContent>
              <div className="space-y-3">
                {[
                  { label: "Org (Cat I Rooms)", key: "org_cat_i", color: "text-blue-700" },
                  { label: "Org (Cat II Rooms)", key: "org_cat_ii", color: "text-purple-700" },
            ...
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Row label="Org Cat I Room Rent" value={`${report.org_cat_i_days} × ₹${report.rates?.cat_i_room_rent || 470} = ₹${((report.org_cat_i_days || 0) * (report.rates?.cat_i_room_rent || 470)).toFixed(2)}`} />
                  <Row label="Org Cat II Room Rent" value={`${report.org_cat_ii_days} × ₹${report.rates?.cat_ii_room_rent || 385} = ₹${((report.org_cat_ii_days || 0) * (report.rates?.cat_ii_room_rent || 385)).toFixed(2)}`} />
                  <Row label="Non-Org Room Rent" value={`${report.non_org_days} × ₹${report.rates?.non_org_room_rent || 570} = ₹${((report.non_org_days || 0) * (report.rates?.non_org_room_rent || 570)).toFixed(2)}`} />
